<template>
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="250" class-name="fixed-width">
        <template slot-scope="{row,$index}" v-if=" row.id != 1">
          ...
          <el-button type="primary" size="mini" @click="handleStaff(row)">
            员工列表
          </el-button>
    ...
    <!-- 员工列表 -->
    <staff-detail ref="staffDetailChild" />
  </div>
</template>

<script>
import { getHotelRoleList, getRoleAdd, getRoleEdit, getRoleDel,getTreeList } from '@/api/system-manage/role-list'
import Pagination from '@/components/Pagination'
import StaffDetail from './components/staff-detail'

import mix from '@/mixs/mix'
export default {
  components: { Pagination,StaffDetail},
  mixins: [mix],
  data() {
    return {
      listQuery: {
  ...
  methods: {
    ...
    getList() {
      ...
      getHotelRoleList(this.listQuery).then(res => {
    ...
    //获取树形菜单
    getTreeData(){
      getTreeList({menu_type:'merch'}).then(res=>{
    ...
    // 员工列表
    handleStaff(row){
      this.$refs.staffDetailChild.init(row)
    }
  }
}
</script>
