...
<script>
import { getBackstageRoleList, getRoleAdd, getRoleEdit, getRoleDel,getTreeList } from '@/api/system-manage/role-list'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getBackstageRoleList(this.listQuery).then(res => {
    ...
    handleUpdate(item) {
      ...
      this.$nextTick(()=>{
        this.$refs.tree.setCheckedKeys([]);
        this.defaultCheckedTreeShow = arr
    ...
    //获取树形菜单
    getTreeData(){
      getTreeList({menu_type:'back'}).then(res=>{
    ...
    getTreeCheck(arr){
      if(arr.length>0){
        ...
        arr.map((item1,i)=>{
          if(item1.children){
            checkedTreeALL.push(item1.id)
            this.getTreeCheck(item1.children)
          }else{
            checkedTreeALL.push(item1.id)
          }
        })
        console.log(checkedTreeALL)
        this.$refs.tree.setCheckedKeys(checkedTreeALL)
      }
    },
    // 点击全部展开还是全部隐藏
    expandItem(flag) {
